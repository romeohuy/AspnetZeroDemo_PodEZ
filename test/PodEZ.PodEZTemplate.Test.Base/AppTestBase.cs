...
using PodEZ.PodEZTemplate.Test.Base.TestData;

namespace PodEZ.PodEZTemplate.Test.Base
{
    /// <summary>
    /// This is base class for all our test classes.
    /// It prepares ABP system, modules and a fake, in-memory database.
    /// Seeds database with initial data.
    /// Provides methods to easily work with <see cref="PodEZTemplateDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(PodEZTemplateDbContext context)
            {
        ...
        protected void UsingDbContext(Action<PodEZTemplateDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PodEZTemplateDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<PodEZTemplateDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<PodEZTemplateDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PodEZTemplateDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PodEZTemplateDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PodEZTemplateDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PodEZTemplateDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<PodEZTemplateDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<PodEZTemplateDbContext, Task<TResult>> func)
        {
            TResult result;
