...
using PodEZ.PodEZTemplate.Web.Areas.App.Models.CustomizableDashboard;
...
{
    public abstract class CustomizableDashboardControllerBase : PodEZTemplateControllerBase
    {
        ...
        public async Task<PartialViewResult> AddWidgetModal(string dashboardName, string pageId)
        {
            var userDashboard = await DashboardCustomizationAppService.GetUserDashboard(
                new GetDashboardInput
                {
                    ...
                    Application = PodEZTemplateDashboardCustomizationConsts.Applications.Mvc
                }
            );
